...
class theme_customizer {
	...
	public function __construct($data) {
		$this->data = $data;
		add_action('customize_register',array($this,'prefab_customize_register'));
	...
	function custom_admin_style($hook) {
		if ( 'customize.php' == $hook || 'widgets.php' == $hook ) {
			wp_enqueue_style( 'prefab-control-admin-css', get_template_directory_uri() . '/inc/css/prefab-customizer.css',array());
			wp_enqueue_script( 'prefab-control-admin-js', get_template_directory_uri() . '/inc/js/prefab-customizer.js', array( 'jquery' ), '20150611', true );
		}
	}	
	public function prefab_customize_register($customizeObj) {
	...
	public function create_control($value,$key) {
		...
		switch($value['type']){
			...
			case "switch":
				$args['type'] = $value['type'];
				$inst = new prefab_Customize_Switch_Control($this->customizeObj,$id,$args);
				break;
			case "image":
	...
	public static function prefab_data_load(){
		$data = apply_filters( 'theme_customizer_data', array());
		if(!empty($data) && is_array($data)){
			new self($data);
		}
	}
}
add_action('init','theme_customizer::prefab_data_load');


if(class_exists( 'WP_Customize_control')):
	class Prefab_Customize_Switch_Control extends WP_Customize_Control {
		public $type = 'switch';
		public function render_content() {
			...
			<label>
				...
				<div class="switch_options">
					<span class="switch_enable"><?php esc_html_e('Yes','prefab'); ?></span>
					<span class="switch_disable"><?php esc_html_e('No','prefab'); ?></span>  
